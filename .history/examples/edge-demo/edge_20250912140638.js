(() => {
  ...
  state: { data: null, nodes: [], panelOpen: false, panelEl: null, openedOnce: false, lastHref: '', lastSig: '', refreshTimer: null, inFlightAbort: null, inFlightTimer: null, sigDebounceTimer: null, pollTimer: null },
    ...
    // Client-side fingerprinting removed - now handled server-side only
    getBasicPageInfo() {
      return {
        url: location.href,
    ...
    // Removed: extractFromJsonLdNode - fingerprinting now handled server-side

    // Removed: extractBrandFromTitle - fingerprinting now handled server-side

    // Removed: extractStyleCode - fingerprinting now handled server-side

    // Removed: extractCurrentPrice - fingerprinting now handled server-side

    // Removed: parsePrice - fingerprinting now handled server-side

    // Removed: parsePriceRange - fingerprinting now handled server-side

    // Removed: extractPriceFromDescription - fingerprinting now handled server-side

    // Removed: detectSelectedVariant - fingerprinting now handled server-side

    // Removed: All size extraction methods - fingerprinting now handled server-side
