(() => {
            ...
            if (data.nodes && data.nodes.length > 0) {
              ...
              data.nodes.forEach((store, index) => {
                console.log(`   ${index + 1}. ${store.name}:`, {
                  ...
                  profit: `$${store.margin?.toFixed(2)}`, // Internal field name kept for compatibility
  ...
  installWatchers() {
    ...
    injectStyles() {
      const css = `
        ...
        /* Input styling - aligned and consistent */
        .zip-input, .jenni-edge-panel button {
  ...
  async openPanel(){
      ...
      const profitTitle = pg ? (this.config.debug ? `Margin $${Math.round(pg.margin||0)} vs target $${Math.round(pg.floor||0)}` : `Save $${Math.round(pg.margin||0)}`) : '';
      ...
      panel.innerHTML = `
        ...
        <div class="jenni-edge-node loading">Finding nearby stores…</div>
          ...
          <div style="display:flex;gap:8px;align-items:center;margin-bottom:8px">
            <input aria-label="ZIP code" class="zip-input" placeholder="ZIP" value="${this.config.zip}"/>
            <button class="zip-apply">Update</button>
          </div>
      ...
      try {
        ...
        try {
          ...
          if (formulaEl && pgNow) {
            ...
            formulaEl.textContent = this.config.debug ? 
              `PDP $${r(pgNow.price)} → Buy $${r(pgNow.buy_cost||pgNow.landed_cost)} + Courier $${r(pgNow.courier_est)} + Fee $${r(pgNow.fee)} = Profit $${r(pgNow.margin)}` :
              `You save $${r(pgNow.margin)} with JENNi delivery`;
          }
        } catch {}
  ...
  renderNodes(container, nodes){
    ...
    empty.textContent = reason === 'no_nearby_stores' ? 'No nearby stores found.' : 'No local availability — showing fastest shipping.';
      ...
      nodes.slice(0,3).forEach(n => {
        ...
        // Determine best link for the store
        let linkHref = '';
        let linkLabel = '';
        
        if (n.productUrl && /^https?:/i.test(n.productUrl)) {
          linkHref = n.productUrl;
          linkLabel = '🛍️ View Product';
        } else if (n.website && /^https?:/i.test(n.website)) {
          linkHref = n.website;
          linkLabel = '🏪 Store Website';
        } else {
          linkHref = `https://www.google.com/search?q=${encodeURIComponent(n.name + ' ' + (this.state.data?.product?.styleCode || this.state.data?.product?.title || 'store'))}`;
          linkLabel = '🔍 Search Store';
        }
        
        const nameHtml = `<a href="${linkHref}" target="_blank" rel="noopener" style="color:#0f766e;text-decoration:none;font-weight:600" title="${linkLabel}">${n.name}</a>`;
        
        // Additional store info
        const storeInfo = [];
        if (n.distanceMiles) storeInfo.push(`${Math.round(n.distanceMiles * 10) / 10} mi`);
        if (n.etaMinutes) storeInfo.push(`~${Math.round(n.etaMinutes)}m`);
        if (n.score) storeInfo.push(`Score: ${(n.score * 100).toFixed(0)}%`);
        
        row.innerHTML = `
          <div class="name">${nameHtml}${pass}${productMatch}</div>
          <div class="meta">${storeInfo.join(' • ')}</div>
          <div class="meta">${this.config.debug ? 'Profit' : 'Save'}: $${Math.round(n.margin||0)}${this.config.debug ? ' | Floor: $' + Math.round(n.floor||0) : ''}</div>
          <div class="meta" style="font-size:10px;color:#6b7280;margin-top:2px">
            <a href="${linkHref}" target="_blank" rel="noopener" style="color:#3b82f6;text-decoration:none">${linkLabel}</a>
          </div>
        `;
    ...
    updatePanelContent(panel, data){
      ...
      try {
        ...
        if (formulaEl && data?.profitGuard) {
          ...
          const r = (n)=> Number.isFinite(n)?Math.round(n):'-';
          formulaEl.textContent = this.config.debug ? 
            `PDP $${r(pg.price)} → Buy $${r(pg.buy_cost||pg.landed_cost)} + Courier $${r(pg.courier_est)} + Fee $${r(pg.fee)} = Profit $${r(pg.margin)}` :
            `You save $${r(pg.margin)} with JENNi delivery`;
        }
