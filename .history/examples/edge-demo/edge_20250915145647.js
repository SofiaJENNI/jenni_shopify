  ...
  renderNodes(container, nodes){
    ...
    empty.textContent = reason === 'no_nearby_stores' ? 'No nearby stores found.' : 'No local availability — showing fastest shipping.';
      ...
      nodes.slice(0,3).forEach(n => {
        ...
        // Determine best link for the store (prefer product URL)
        let linkHref = '';
        ...
        const nameHtml = linkHref ? 
          `<a href="${linkHref}" target="_blank" rel="noopener" style="color:#0f766e;text-decoration:none;font-weight:600" title="View at store">${storeName}</a>` :
