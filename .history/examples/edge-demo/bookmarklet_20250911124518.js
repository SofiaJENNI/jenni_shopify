javascript:(function(){
  ...
  s.onload=function(){ 
    try{
      ...
      // Show success message with fingerprint tip
      setTimeout(function(){
        if(window.JenniEdge && window.JenniEdge.state && window.JenniEdge.state.panelOpen){
          console.log('✅ JENNi loaded! Click 🔍 in panel to see fingerprint quality.');
        } else {
          console.log('✅ JENNi loaded! Click the delivery pill to open panel, then 🔍 for fingerprint.');
        }
      }, 1000);
    }catch(e){
