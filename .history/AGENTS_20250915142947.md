...
- `src/core/eligibility.ts` — JENNi auth, product lookup, in-memory caching.
...
- Copy env: `cp .env.example .env` and set:
  ...
  - Optional: `REDIS_URL` (for order queue only), `JENNI_ORDERS_URL`, `JENNI_API_KEY`
...
- Caching: In-memory TTL is 10m in `eligibility.ts`; don't change semantics casually.
