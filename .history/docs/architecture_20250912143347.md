...
- Edge demo API (`src/routes/edge.ts`):
  - GET `/edge/eligible?url=&zip=&detail=1&debug=1` → fetch PDP, server-side fingerprint extraction, run resolve+ProfitGuard; returns product-led fields (matchType, substitutionLevel, suggestedIncentive, waitlistOffered, nodes Pass/Hold)
  - POST `/edge/resolve` → like eligible but expects URL for server-side fingerprint generation
...
- `src/routes/edge.ts` – All demo endpoints, server-side PDP fingerprinting with size/color extraction, Google Places integration, resolve/economics, preview overlay HTML, client/loader/embed delivery.
