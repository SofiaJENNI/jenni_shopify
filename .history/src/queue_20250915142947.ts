...
async function initRedis(): Promise<boolean> {
  ...
  initPromise = (async () => {
    try {
      ...
      if (!redisUrl) {
        console.log('⚠️  No REDIS_URL configured, queue functionality disabled');
...
// Add function to queue orders with lazy initialization
export async function queueOrder(name: string, data: any) {
  await initRedis(); // Ensure Redis is initialized when actually needed
  return orderQueue?.add(name, data);
}
